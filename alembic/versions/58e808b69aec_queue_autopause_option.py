...
def upgrade():
    ...
    op.execute(qry)
    qry = """ALTER TABLE queue ALTER COLUMN autopause SET DEFAULT 'yes';"""
    op.execute(qry)
    qry = """UPDATE queue SET autopause='yes' WHERE autopause='1';"""
    op.execute(qry)
    qry = """UPDATE queue SET autopause='no' WHERE autopause='0';"""
    op.execute(qry)
    qry = """ALTER TABLE queue ADD CONSTRAINT queue_autopause CHECK (autopause IN ('yes','no','all'));"""
    op.execute(qry)
...
def downgrade():
    ...
    op.execute(qry)
    qry = """UPDATE queue SET autopause='1' WHERE autopause='yes';"""
    op.execute(qry)
    qry = """UPDATE queue SET autopause='1' WHERE autopause='all';"""
    op.execute(qry)
